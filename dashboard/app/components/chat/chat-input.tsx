...
export function ChatInput({ onSend, status }: ChatInputProps) {
  ...
  const disabled = status !== 'connected' || !value.trim();
    ...
    <form onSubmit={handleSubmit} className="flex items-center gap-2 border-t border-border p-4">
      <div className="relative flex-1">
        <textarea
          ...
          placeholder={status === 'connected' ? 'Send a message...' : 'Connecting...'}
      ...
      <Button type="submit" size="icon" disabled={disabled} className="shrink-0 glow-cyan">
