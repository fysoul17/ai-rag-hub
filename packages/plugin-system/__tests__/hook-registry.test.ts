...
describe('HookRegistry', () => {
  ...
  describe('register()', () => {
    test('adds a handler for a hook type', () => {
      ...
      registry.register(HookType.BEFORE_MESSAGE, handler);
      expect(registry.getHandlerCount(HookType.BEFORE_MESSAGE)).toBe(1);
    });

    test('returns an unsubscribe function', () => {
      const handler = makeHookHandler();
      const unsub = registry.register(HookType.BEFORE_MESSAGE, handler);
      expect(typeof unsub).toBe('function');

      unsub();
      expect(registry.getHandlerCount(HookType.BEFORE_MESSAGE)).toBe(0);
    });

    test('registers multiple handlers for the same hook type', () => {
      registry.register(HookType.BEFORE_MESSAGE, makeHookHandler());
      registry.register(HookType.BEFORE_MESSAGE, makeHookHandler());
      registry.register(HookType.BEFORE_MESSAGE, makeHookHandler());
      expect(registry.getHandlerCount(HookType.BEFORE_MESSAGE)).toBe(3);
    });

    test('registers handlers for different hook types', () => {
      registry.register(HookType.BEFORE_MESSAGE, makeHookHandler());
      registry.register(HookType.AFTER_RESPONSE, makeHookHandler());
      expect(registry.getHandlerCount(HookType.BEFORE_MESSAGE)).toBe(1);
    ...
    test('accepts a pluginId option', () => {
      registry.register(HookType.BEFORE_MESSAGE, makeHookHandler(), { pluginId: 'plugin-a' });
      expect(registry.getHandlerCount(HookType.BEFORE_MESSAGE)).toBe(1);
    });
  ...
  describe('unsubscribe / unregister', () => {
    test('unsubscribe removes only the specific handler', () => {
      ...
      const unsub1 = registry.register(HookType.BEFORE_MESSAGE, handler1);
      registry.register(HookType.BEFORE_MESSAGE, handler2);
    ...
    test('unregisterPlugin removes all hooks for a plugin', () => {
      registry.register(HookType.BEFORE_MESSAGE, makeHookHandler(), { pluginId: 'plugin-a' });
      registry.register(HookType.AFTER_RESPONSE, makeHookHandler(), { pluginId: 'plugin-a' });
      registry.register(HookType.BEFORE_MESSAGE, makeHookHandler(), { pluginId: 'plugin-b' });

      registry.unregisterPlugin('plugin-a');
      expect(registry.getHandlerCount(HookType.BEFORE_MESSAGE)).toBe(1); // plugin-b's
      expect(registry.getHandlerCount(HookType.AFTER_RESPONSE)).toBe(0);
    });
    ...
    test('clear removes all handlers', () => {
      registry.register(HookType.BEFORE_MESSAGE, makeHookHandler());
      registry.register(HookType.AFTER_RESPONSE, makeHookHandler());
      registry.register(HookType.BEFORE_AGENT_CREATE, makeHookHandler());
  ...
  describe('emit() — fire-and-forget', () => {
    test('calls all registered handlers for the hook type', async () => {
      const handler1 = makeHookHandler();
      const handler2 = makeHookHandler();
      registry.register(HookType.BEFORE_AGENT_DELETE, handler1);
      registry.register(HookType.BEFORE_AGENT_DELETE, handler2);

      await registry.emit(HookType.BEFORE_AGENT_DELETE, { agentId: 'agent-1' });
    ...
    test('passes the event data to handlers', async () => {
      ...
      registry.register(HookType.BEFORE_AGENT_DELETE, handler);
    ...
    test('awaits async handlers', async () => {
      let completed = false;
      registry.register(HookType.AFTER_AGENT_CREATE, async () => {
        await delay(20);
        completed = true;
      });
    ...
    test('handler throwing does not prevent other handlers from executing', async () => {
      ...
      registry.register(HookType.AFTER_AGENT_CREATE, handler2, { priority: 2 });

      // Should not throw
      await registry.emit(HookType.AFTER_AGENT_CREATE, { test: true });
    ...
    test('async handler rejection is caught and contained', async () => {
      const handler2 = makeHookHandler();
      registry.register(
        HookType.BEFORE_AGENT_DELETE,
        async () => {
          throw new Error('async boom');
        },
        { priority: 1 },
      );
      registry.register(HookType.BEFORE_AGENT_DELETE, handler2, { priority: 2 });

      // Should not throw
      await registry.emit(HookType.BEFORE_AGENT_DELETE, {});

      expect(handler2.calls).toHaveLength(1);
    });

    test('emit still completes even if all handlers throw', async () => {
      registry.register(HookType.BEFORE_MEMORY_STORE, () => {
        throw new Error('a');
      });
      registry.register(HookType.BEFORE_MEMORY_STORE, () => {
        throw new Error('b');
      ...
      // Should not throw
      await registry.emit(HookType.BEFORE_MEMORY_STORE, {});
    });
  });
  ...
  describe('emitWaterfall() — data transformation', () => {
    test('passes output of handler N as input to handler N+1', async () => {
      ...
      const result = await registry.emitWaterfall(HookType.BEFORE_MESSAGE, { content: 'start' });
    ...
    test('handler returning undefined passes data through unchanged', async () => {
      ...
      const result = await registry.emitWaterfall(HookType.BEFORE_MESSAGE, { content: 'hello' });
    ...
    test('returns original data when no handlers registered', async () => {
      ...
      const result = await registry.emitWaterfall(HookType.BEFORE_MESSAGE, data);
    ...
    test('waterfall respects priority order', async () => {
      ...
      const result = await registry.emitWaterfall(HookType.AFTER_RESPONSE, { content: 'start' });
    ...
    test('waterfall works with async handlers', async () => {
      ...
      const result = await registry.emitWaterfall(HookType.BEFORE_MESSAGE, { count: 5 });
      // (5 + 1) * 2 = 12
  ...
  describe('getHandlerCount()', () => {
    ...
    test('returns 0 for hook type with no handlers', () => {
      expect(registry.getHandlerCount(HookType.BEFORE_MEMORY_STORE)).toBe(0);
    });
  });
});
